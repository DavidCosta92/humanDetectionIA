...
while True:
    ...
    # originales de pileta => pISx = 0
    # originales de pileta => pISy = 280
    # originales de pileta => pDSx = 600
    # originales de pileta => pDSy = 125
    # originales de pileta => pIIx = 230
    # originales de pileta => pIIy = 600
    # originales de pileta => pDIx = 1000
    # originales de pileta => pDIy = 260
    ...
    if personDetected is not None:# TODO ACA DEBERIA METER LA VALIDACION DE PERSONA, ES PERSONA? ES ADULTO? ES NIÑO?
        personasDetectadas.append(personDetected)


    info_id = seguimiento.rastreo(personasDetectadas)

    print("PEPRSONA DETECTADAs  => " + str(personasDetectadas))
    print("Persona rastreada  => " + str(info_id))

    #    cv2.rectangle(frame, (x, y - 15), (x + ancho, y + alto), (100, 200, 30), 5)  # dibujamos el de los falsos positivos

    cv2.putText(frame, "info_id  => " + str(info_id), (100 , 100), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 0), 4)
    ...
    for inf in info_id:
        ...
        """
        if zonaAgua >= 0:
            cv2.rectangle(frame , (x,y-15), (x+ancho,y+alto), (0, 0,255 ) , 3) #dibujamos el rectangulo rojo
        elif zonaPileta == 1:
            cv2.rectangle(frame , (x-10,y-10), (x+ancho,y+alto), (0, 255 ,0 ) , 2) #dibujamos el rectangulo verde
        """
